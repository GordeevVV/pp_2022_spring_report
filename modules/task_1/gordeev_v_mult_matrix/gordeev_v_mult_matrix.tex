...
\par В файле исходного кода mult\_matrix.cpp содержится реализация функций, объявленных в заголовочном файле. В файле исходного кода main.cpp содержатся тесты для проверки корректности программы.
\newpage
...
TEST(Gordeev\_Mult\_Matrix, Can\_Mult\_Matrix\_Works\_Correct) {
    MultMatrix mtx1(3, 5);
    MultMatrix mtx2(5, 6);
    EXPECT\_NO\_THROW(mtx1.multMatrix(mtx2));
}

TEST(Gordeev\_Mult\_Matrix, Cant\_Mult\_Wrong\_Size\_Matrix) {
    MultMatrix mtx1(2, 9);
    MultMatrix mtx2(4, 5);
    EXPECT\_ANY\_THROW(mtx1.multMatrix(mtx2));
}
...
TEST(Gordeev\_Mult\_Matrix, Time\_Test) {
